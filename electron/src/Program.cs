...
namespace ArcGisMaps
{
    internal class Program
    {
        ...
        private static int _signInInProgress = 0;
        ...
        static void Main(string[] args)
        {
            ...
            {
                string portalUrl = options.portalUrl;
                string clientId = options.clientId;
                string redirectUrl = options.redirectUrl;

                if (string.IsNullOrWhiteSpace(portalUrl))
                    return Task.FromException(new Exception("No portalUrl provided"));
                if (string.IsNullOrWhiteSpace(clientId))
                    return Task.FromException(new Exception("No clientId provided"));
                if (string.IsNullOrWhiteSpace(redirectUrl))
                    return Task.FromException(new Exception("No redirectUrl provided"));
                if (Interlocked.Exchange(ref _signInInProgress, 1) == 1)
                    return Task.FromException(new Exception("A sign-in flow is already in progress"));

                try
                {

                    SetChallengeHandler(portalUrl, clientId, redirectUrl);
                ...
                {
                    return Task.FromException(new Exception("User cancelled"));
                } catch (Exception ex)
                {
                    return Task.FromException(new Exception("Sign-in failed"));
                } finally
                {
            ...
            connection.OnAsync("signOut", () =>
            {
                ...
                {
                    return Task.FromException(new Exception($"Failed to sign out: {ex.Message}"));
                }
            });

            connection.OnAsync("query", (dynamic options) =>
            {
                return Task.FromResult("Querying...");
            });

            connection.Listen();
        }

        private static void SetChallengeHandler(string portalUrl, string clientId, string redirectUrl)
        {
            var userConfig = new OAuthUserConfiguration(new Uri(portalUrl), clientId, new Uri(redirectUrl));
