...
class ArcGisMapsPlugin : Plugin() {
  ...
  private object GeoJsonUtil {
    fun toJson(g: com.arcgismaps.geometry.Geometry?): JSObject? =
      when (g) {
        null -> null
        is com.arcgismaps.geometry.Point -> point(g)
        is com.arcgismaps.geometry.Polyline -> line(g)
        else -> null
      }

    private fun point(p: com.arcgismaps.geometry.Point) =
      JSObject().apply {
        put("x", p.x)
        put("y", p.y)
      }

    private fun line(pl: com.arcgismaps.geometry.Polyline): JSObject {
      val paths = JSArray()
